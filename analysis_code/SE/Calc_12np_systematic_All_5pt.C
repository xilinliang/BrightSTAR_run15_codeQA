void Calc_12np_systematic_All_5pt()
{

	ifstream com_sig;
	com_sig.open("ANtext/Run15FMSfinal_12np_West_result.txt");
	ifstream SBBC;
	SBBC.open("ANtext/Run15FMSfinal_12np_West_Sys_smallBBC.txt");
	ifstream LBBC;
	LBBC.open("ANtext/Run15FMSfinal_12np_West_Sys_largeBBC.txt");
	ifstream ROF;
	ROF.open("ANtext/Run15FMSfinal_12np_West_Sys_ROF.txt");
	ifstream Esum;
	Esum.open("ANtext/Run15FMSfinal_12np_West_Sys_Esum.txt");
	ifstream bkg;
	bkg.open("ANtext/frac_sys_SE.txt");
	//ifstream Dilution;
	//Dilution.open("ANtext/Run15FMScase23_EnergyCorrectionCompare4.txt");
	//ifstream Esum;
	//Esum.open("ANtext/Run15FMSfinal_12np_WestSys_Esum.txt");
	//ifstream MixBkg;
	//MixBkg.open("frac_sys.txt");

	ofstream resultfile;
	resultfile.open("finalANtext/run15_FMS_plot_SE_data_12np_5pt.txt");
	ofstream Bsysunfile;
	Bsysunfile.open("finalANtext/run15_FMS_BSystematic_SE_summary_12np_5pt.txt");
	ofstream Ysysunfile;
	Ysysunfile.open("finalANtext/run15_FMS_YSystematic_SE_summary_12np_5pt.txt");
	...
	Bsysunfile << "Xpoint	small BBC 	large BBC	Ring of Fire	E sum 	Background	sum" << endl;
	Ysysunfile << "Xpoint	small BBC 	large BBC	Ring of Fire	E sum 	Background	sum" << endl;

    	TFile *file = TFile::Open("root_result/Run15FMSWestAna/Run15FMSXFCalc_AnaAll.root");   
    	TH1D *h1_xF1020;
    	TH1D *h1_xF2025;
    	TH1D *h1_xF2530;
    	TH1D *h1_xF3035;
	...
	for (int i=0; i< nXFUsed; i=i+1)
	{
		...
		sum_sys_B = sum_sys_B + diff * diff;
		bkg >> XFpointtmp >> frac;
		diff = frac * fabs(com_sig_AN); 
		Bsysunfile <<"	"<<diff;
		sum_sys_B = sum_sys_B + diff * diff;
		sum_sys_B = sqrt(sum_sys_B);
		Bsysunfile <<"  "<<sum_sys_B<<endl;
		cout << "B" << "	" << XFpoint[i+1] << "	" << sum_sys_B <<endl;
		resultfile << XFpoint[i+1] << "	" << com_sig_AN <<"	"<< com_sig_sta << "	" << sum_sys_B  <<endl;

		sum_sys_Y = 0;
		com_sig >> beam >> XFpointtmp >> com_sig_AN >> com_sig_sta;
		SBBC >> beam >> XFpointtmp >> diff;
		Ysysunfile << XFpointYellow[i+1] <<"	" << diff;
		...
		cout << "Y" << "	" << XFpointYellow[i+1] << "	" << sum_sys_Y <<endl;
		resultfile << XFpointYellow[i+1] << "	" << com_sig_AN <<"	"<< com_sig_sta << "	" << sum_sys_Y  <<endl;
