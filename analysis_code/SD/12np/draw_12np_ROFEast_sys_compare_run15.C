void draw_12np_ROFEast_sys_compare_run15()
{
	...
	ofstream outputtext;
	outputtext.open("ANtext/Run15FMSfinal_12np_East_Sys_ROF.txt");
	ofstream outputtext_no;
	outputtext_no.open("ANtext/Run15FMSfinal_12np_East_Sys_ROF_noconstrain.txt");

    	ifstream inputtext;
    	//inputtext.open("ROFW450_12np.txt");
    	inputtext.open("ANtext/Run15FMSfinal_12np_East_result.txt");
	...
	inputtext >> beam >> XFpoint >> YellowAN_4045[1] >> YellowANErr_4045[1]  ;
    	...
    	inputtext.open("ANtext/Run15FMSfinal_12np_East_SysROF.txt");
...
//	//YellowANDiff_un_3045[4] = sqrt(fabs(pow(YellowANErr_3045[4] , 2) - pow(YellowANErr_3045[2] ,2)));
	...
	TH2D * temp = new TH2D("temp","Blue beam A_{N}",8,0.15,0.45,30,-0.1,0.1);
	...
	//leg->Draw("same");
//
	ANplot->SaveAs("fig/sys/FMS_BlueAN_ROFstudy_East_12np_Sys.pdf");
...
//
	TCanvas *ANplot = new TCanvas("ANPlot","",1024,768);
	ANplot->DrawFrame(2., -0.06, 250., 0.2);
	TH2D * temp = new TH2D("temp","Yellow beam A_{N}",8,0.15,0.45,30,-0.1,0.1);
	...
	ANplot->SaveAs("fig/sys/FMS_YellowAN_ROFstudy_East_12np_Sys.pdf");
