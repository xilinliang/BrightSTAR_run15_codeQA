...
void DiffJetXFANCalc_6pt_East_getChi2AndConst(TString inFileName, TString outNameText, TString photon, TString process, TString det)
{
    ...
    int nDrawPoints = 5;
    int StartBin = 4;
    if (process == "RG") {nDrawPoints = 6; StartBin = 3;}
	...
	for (int j=StartBin; j<nXFusebins; j=j+1)
	{
		...
		cout << "ndf: " << BlueFit->GetNDF() << endl;

		outputtext << "B" << "	" << XFpoint[j-3] << "	" << BlueFit->GetChisquare() << "		" << (1.0 * BlueFit->GetParameter(1) / BlueFit->GetParError(1)) << endl;
		...
		outputtext << "Y" << "	" << XFpoint[j-3] << "	" << YellowFit->GetChisquare() << "		" << (1.0 * YellowFit->GetParameter(1) / YellowFit->GetParError(1)) << endl;
